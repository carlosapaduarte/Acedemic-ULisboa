...
def get_completed_level_ranks(db: Session, user_id: int, app_scope: str) -> set[int]:
    """
    Calcula e devolve um conjunto com os ranks dos níveis que o utilizador completou a 100%
    """
    all_active_badges = get_all_badges_for_app_scope(db, app_scope=app_scope)
    
    user_badge_associations = get_user_earned_badges(db, user_id)
    earned_badge_ids: set[int] = {ub.badge_id for ub in user_badge_associations}

    badges_by_level = defaultdict(list)
    for badge, league in all_active_badges:
        if league.rank > 0:
            badges_by_level[league.rank].append(badge.id)

    completed_ranks = set()
    for rank, badge_ids_in_level in badges_by_level.items():
        level_badge_set = set(badge_ids_in_level)
        if earned_badge_ids.issuperset(level_badge_set):
            completed_ranks.add(rank)
            
    return completed_ranks


def update_login_streak(db: Session, user_id: int) -> List[Badge]: # Adicionado o tipo de retorno
    """Atualiza a login streak, avalia medalhas e ligas e devolve as medalhas ganhas"""
...
def get_all_badges_for_app_scope(db: Session, app_scope: str) -> List[Tuple[Badge, League]]:
    """ procura todas as medalhas ativas, retorna um par (Badge, League) para cada resultado"""
    query = (
        select(Badge, League)
        .join(League, Badge.league_id == League.id)
...
def get_user_badge_history(db: Session, user_id: int) -> List[UserBadge]:
    ...
    statement = (
        ...
        .options(selectinload(UserBadge.badge).selectinload(Badge.league))
