...
function useHowMuchEnergyQuestionPage(onQuestionSubmitted: () => void) {
    ...
    service
      ...
      .then((energyHistoryDaysStatus) => {
        if (energyHistoryDaysStatus.length > 0) {
          const lastEnergyDayStatus =
            energyHistoryDaysStatus[energyHistoryDaysStatus.length - 1];
  ...
  function onConfirmPressHandler() {
    function getCurTimeOfDay(): TimeOfDay {
      ...
      if (hour < 19) return TimeOfDay.AFTERNOON;
    ...
    service
      .createDailyEnergyStat(level, timeOfDay)
      .then(() => {
...
export function HowMuchEnergyQuestionPage({
  onComplete,
}: {
  onComplete: () => void;
}) {
  ...
  return (
    <div className={styles.energyQuestionPage}>
      ...
      <div className={styles.selectedEnergyLevelIconContainer}>
        <img
          src={getEnergyIconByEnergyLevel(energyLevel)}
          alt="Energy Status Icon"
      ...
      <div className={styles.slideContainer}>
        ...
        <div className={styles.sliderLegend}>
          <span>{t("statistics:tired")}</span>
          <span>{t("statistics:energetic")}</span>
        </div>
      </div>
      ...
      <div className={styles.buttonsContainer}>
        <Button onPress={onConfirmPressHandler} className={styles.saveButton}>
          {t("statistics:save")}
        </Button>
