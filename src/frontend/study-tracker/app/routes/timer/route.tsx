...
function useTimerSetup() {
  ...
  function onTimerFinish() {
    if (timerStopDate == studyStopDate) setTimerStopDate(pauseStopDate);
    else setTimerStopDate(undefined);
  }
...
function useAssociatedTasks() {
  const [associatedTasks, setAssociatedTasks] = useState<Task[] | undefined>(
    undefined
  );
...
function AssociatedTaskListView({
  ...
  const newTaskButtonMsg = showCreateTask
...

{
  ...
  const timerTitleMsg =
    happeningStudyBlock.title + studyStopDate ? "(Study Time)" : "(Pause Time)";
