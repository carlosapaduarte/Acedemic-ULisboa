...
function GoalBox({goalIndex, achieved, onGoalClick} : {goalIndex: number, achieved: boolean, onGoalClick: (goalIndex: number) => void}) {
...
function GoalRow({rowIndex, currentGoal, onGoalClick} : {rowIndex: number, currentGoal: number, onGoalClick: (goalIndex: number) => void}) {
    return (
...
function Goals({goals, onGoalClickHandler} : {goals: Goal[][], onGoalClickHandler: (goalIndex: number) => void}) {
    return (
        <Box display="flex" flexDirection="column" justifyContent="center" marginTop="2%" width="70%">
            {Array.from(Array(ROWS)).map((_, rowIndex: number) => 
                <GoalRow key={rowIndex} currentGoal={goals.length} rowIndex={rowIndex} onGoalClick={onGoalClickHandler}/>
            )}
        </Box>
    )
}
    ...
    return (
        <Box marginTop="2%">
            ...
            <Box height="100%" display="flex" flexDirection="row">
                ...
                <Box display="flex" flexDirection="column" justifyContent="center" width="30%" padding="1%">
                {
                    goalsInfoToDisplay ? 
