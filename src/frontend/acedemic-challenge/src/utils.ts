...
function getGoalsPerDayByStartDate(level: LevelType, startDate: Date): Goal[][] {
    const allGoals: Goal[][] = getGoalsPerDay(level)
    //console.log("All goals: ", allGoals)
    const numberOfPassedDays = Math.round((new Date().getTime() - startDate.getTime()) / (1000 * 3600 * 24))
